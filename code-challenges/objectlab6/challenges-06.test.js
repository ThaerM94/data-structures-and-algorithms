...
const getHouses = (arr) => {
  ...
  arr.forEach(element => {
      ...
      let gameOfThrones = Object.values(element);
    //['Jon S', null , [] , sNOW]
    houses.push(gameOfThrones[3]);
});
  return houses;
};
...
const hasChildrenValues = (arr, character) => {
    let result ;
  arr.forEach(element => {
     let got =   Object.values(element)
     // [name,spouse,chil[],house]
    if (got[0]== character) {
        if (got[2].length>0) {
            result = true ;
        } else {
...
const hasChildrenEntries = (arr, character) => {
    let result ;
    arr.forEach(element => {
       let got =   Object.entries(element)
       // [[name,gg],[spouse,ll],[chil,[]],[house,hh]]
      if (got[0][1]== character) {
          if (got[2][1].length>0) {
              result = true ;
          } else {
              result = false ;
